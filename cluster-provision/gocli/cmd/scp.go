...
const sshKey_x86 = `-----BEGIN RSA PRIVATE KEY-----
...
const sshKey_s390x = `-----BEGIN OPENSSH PRIVATE KEY-----
...
func NewSCPCommand() *cobra.Command {
	...
	_, sshUser := utils.GetSSHCredentialsByArchitecture(runtime.GOARCH)

	ssh.Flags().String("container-name", "dnsmasq", "the container name to SSH copy from")
	ssh.Flags().String("ssh-user", fmt.Sprintf("%s", sshUser), "the user that used to connect via SSH to the node")
