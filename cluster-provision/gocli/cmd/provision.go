...
func provisionCluster(cmd *cobra.Command, args []string) (retErr error) {
	...
	if strings.Contains(phases, "k8s") {
		...
		err = _cmd(cli, nodeContainer(prefix, nodeName), fmt.Sprintf("ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i vagrant.key %s@192.168.66.101 'mkdir -p /tmp/ceph /tmp/cnao /tmp/nfs-csi /tmp/nodeports /tmp/prometheus /tmp/whereabouts'", sshUser), "Create required manifest directories before copy")
		if err != nil {
			return err
		}
